...
function renderTrend(){
 ...
 $('#trend-chart').innerHTML=`<svg viewBox="0 0 650 200" role="presentation">${report.periods.map((p,i)=>{const v=values[i],h=v==null?0:Math.max(0,v/max*125),x=40+i*160;return `<g><title>${esc(p.label)}: ${fmt(v,kind)}</title><rect x="${x}" y="${160-h}" width="85" height="${h}" rx="5" fill="${p.partial?'#9ac6b3':'#24634f'}" opacity="${p.id===state.period?1:.65}"/><text x="${x+42}" y="${151-h}" text-anchor="middle" fill="#173f34" font-size="11">${fmt(v,kind)}</text><text x="${x+42}" y="185" text-anchor="middle" fill="#62706a" font-size="12">${['June','July','August','Sept 1–9'][i]}</text></g>`;}).join('')}</svg>`;
